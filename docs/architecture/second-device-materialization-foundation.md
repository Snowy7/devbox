...
server-side cursors, production pairing UX, automatic conflict resolution, or Electron UI. The
manual smoke path can use `--mock-key-source-db` so a receiving local context can decrypt a
publisher's locally encrypted objects without printing raw key material. That flag is a test/dev
trust bootstrap, not a production key exchange.
...
Import downloads and decrypts the bundle, downloads all included file blobs into the receiving
`BlobCache`, persists the project/snapshot/manifest rows into the receiving SQLite database, and
updates the receiving local device/project cursor to the imported snapshot id.
