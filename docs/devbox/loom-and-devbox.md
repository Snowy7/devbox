...
The repository now uses top-level `loom/` and `devbox/` areas as the active homes for Rust crates:
...
devbox/crates/devbox-auth     account/session/device trust boundary
devbox/crates/devbox-platform hosted product/platform boundary
devbox/crates/devbox-remote   Devbox-hosted Loom remote implementation
devbox/crates/devbox-api      hosted API boundary
devbox/crates/devbox-cli      Devbox product CLI plus alpha compatibility commands
```

The old alpha compatibility crates live under `devbox/crates/` too, so the existing local snapshot,
