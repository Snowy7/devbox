...
Bindhub is a folder continuity product for developers.
...
plain files. Bindhub should still work.

The source-control primitive underneath Bindhub is codenamed **Loom**. Loom is not the user-facing
...
a compatibility surface because developers use it today, but Git is not the foundation Bindhub is
trying to build.

Start with [Loom And Bindhub](bindhub/loom-and-bindhub.md) before reading older architecture slices.
For current alpha proof, read [Alpha Readiness Evidence](evidence/alpha-readiness.md) and
[Workspace Adapter Alpha](bindhub/workspace-adapters-alpha.md). Run `scripts/mvp-two-device-smoke`
...
The top-level [Loom area](../loom/README.md) and [Bindhub area](../bindhub/README.md) are active crate
