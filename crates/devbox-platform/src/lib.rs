//! Devbox hosted platform boundary.
//!
//! Devbox owns accounts, machines, membership, hosted discovery, and product
...
mod tests {
    ...
    fn platform_memberships_reference_loom_shared_folders() {
        ...
        let folder =
            HostedSharedFolder::new(shared_folder_id, account_id, "Devbox").expect("folder");

        assert_eq!(membership.role, SharedFolderRole::Owner);
        assert_eq!(folder.display_name, "Devbox");
    }
}
