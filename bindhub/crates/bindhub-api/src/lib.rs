//! Bindhub hosted API boundary.
//!
//! This crate provides the PR6 local hosted API used by Loom's hosted remote.
//! Bindhub owns sessions, devices, shared-folder membership, pack storage, and
//! cursor metadata. Loom still owns folder-state semantics inside the packs.

use bindhub_platform::{AccountId, DeviceId, SharedFolderRole};
use bindhub_sync::{
...
impl ApiBoundary {
    pub fn bindhub_hosted_api() -> Self {
        Self {
            owns_accounts: true,
            owns_devices: true,
...
impl fmt::Display for ApiError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Unauthorized => f.write_str("missing or invalid Bindhub session"),
...
impl RemotePackStorage {
    fn from_r2_env() -> ApiResult<Self> {
        let endpoint = optional_env_value("BINDHUB_R2_ENDPOINT");
        let bucket = optional_env_value("BINDHUB_R2_BUCKET");
        let (endpoint, bucket) = match (endpoint, bucket) {
            (Some(endpoint), Some(bucket)) => (endpoint, bucket),
            (None, None) => {
                return Err(ApiError::BadRequest(
                    "BINDHUB_R2_ENDPOINT and BINDHUB_R2_BUCKET are not configured".to_string(),
                ));
            }
            _ => {
                return Err(ApiError::BadRequest(
                    "BINDHUB_R2_ENDPOINT and BINDHUB_R2_BUCKET must be set together".to_string(),
                ));
            }
        };
        let region = optional_env_value("BINDHUB_R2_REGION").unwrap_or_else(|| "auto".to_string());
        let prefix = optional_env_value("BINDHUB_R2_PREFIX");
...
impl PostgresMetadataStore {
    fn connect(database_url: &str) -> ApiResult<Self> {
        let attempts = optional_env_value("BINDHUB_API_DATABASE_CONNECT_ATTEMPTS")
    ...
    fn migrate(client: &mut PostgresClient) -> ApiResult<()> {
        client
            .batch_execute(
                ...
                VALUES (1, 'bindhub-api-postgres-v1')
...
impl LocalBindhubApi {
    ...
    pub fn open_from_env(root: impl AsRef<Path>) -> ApiResult<Self> {
        ...
        let metadata: Arc<dyn ApiMetadataStore> =
            match optional_env_value("BINDHUB_API_METADATA_MODE").as_deref() {
                Some("memory") => Arc::new(MemoryMetadataStore::default()),
                Some(other) => {
                    return Err(ApiError::BadRequest(format!(
                        "BINDHUB_API_METADATA_MODE must be 'memory' or unset, got '{other}'"
                    )))
                }
                None => {
        ...
        let pack_storage: Arc<dyn PackStorage> = if optional_env_value("BINDHUB_R2_ENDPOINT")
            .is_some()
            || optional_env_value("BINDHUB_R2_BUCKET").is_some()
        {
        ...
        Self::open_with_stores(
            ...
            metadata,
            pack_storage,
            env_flag_enabled("BINDHUB_API_ENABLE_LOCAL_DEV_SESSION"),
        )
    }
    ...
    pub fn poll_cli_device_flow(&self, device_code: &str) -> ApiResult<CliDeviceFlowPollResponse> {
        ...
        if flow.expires_at_unix <= now_unix_seconds() {
            return Err(ApiError::BadRequest(
                "machine login flow expired; run bindhub login again".to_string(),
            ));
        }
    ...
    pub fn register_device(
        ...
        self.register_device_for_account(
            &auth.account_id,
            &device_id,
            display_name.unwrap_or("Bindhub device"),
        )?;
        Ok(DeviceResponse {
            ...
            account_id: AccountId::new(auth.account_id.clone())
                .map_err(|error| ApiError::BadRequest(error.to_string()))?,
            display_name: display_name.unwrap_or("Bindhub device").to_string(),
        })
    }
            ...
            match stream {
                Ok(stream) => {
                    ...
                    if let Err(error) = handle_stream(&api, stream) {
                        eprintln!("bindhub-api request failed: {error}");
                    }
                }
                Err(source) => {
...
pub fn spawn_local_test_server(root: impl AsRef<Path>) -> ApiResult<LocalApiServer> {
    let api = LocalBindhubApi::open(root)?.with_local_dev_sessions();
...
fn handle_stream(api: &Arc<LocalBindhubApi>, mut stream: TcpStream) -> ApiResult<()> {
...
fn route_request(api: &LocalBindhubApi, request: HttpRequest) -> ApiResult<Vec<u8>> {
    let path = request
        .path
        .split('?')
        .next()
...
fn auth_from_request(api: &LocalBindhubApi, request: &HttpRequest) -> ApiResult<AuthContext> {
    let token = request
        .headers
        .get("authorization")
    ...
    let device_id = request
        .headers
        .get("x-bindhub-device-id")
        .ok_or(ApiError::Unauthorized)?;
    api.authenticate(token, device_id)
}

fn require_service_token(request: &HttpRequest) -> ApiResult<()> {
    let expected = optional_env_value("BINDHUB_API_SERVICE_TOKEN").ok_or(ApiError::Unauthorized)?;
    let actual = request
        .headers
        .get("x-bindhub-api-service-token")
        .ok_or(ApiError::Unauthorized)?;
    if actual == &expected {
        Ok(())
    } else {
...
fn metadata_database_url() -> ApiResult<String> {
    optional_env_value("BINDHUB_API_DATABASE_URL")
        .or_else(|| optional_env_value("DATABASE_URL"))
        .ok_or_else(|| {
            ApiError::BadRequest(
                "metadata database is required: set DATABASE_URL or BINDHUB_API_DATABASE_URL"
                    .to_string(),
            )
        })
}
...
fn cli_auth_verification_uri() -> String {
    optional_env_value("BINDHUB_CLI_AUTH_VERIFICATION_URL")
...
mod tests {
    ...
    fn api_boundary_delegates_folder_state_to_loom() {
        let boundary = ApiBoundary::bindhub_hosted_api();
    ...
    fn open_from_env_can_use_memory_metadata_for_local_smoke() {
        ...
        let old_mode = std::env::var("BINDHUB_API_METADATA_MODE").ok();
        let old_database_url = std::env::var("BINDHUB_API_DATABASE_URL").ok();
        let old_database_url_fallback = std::env::var("DATABASE_URL").ok();
        std::env::set_var("BINDHUB_API_METADATA_MODE", "memory");
        std::env::remove_var("BINDHUB_API_DATABASE_URL");
        std::env::remove_var("DATABASE_URL");
        let dir = tempfile::tempdir().expect("temp dir");

        let api =
            LocalBindhubApi::open_from_env(dir.path()).expect("api opens with memory metadata");

        assert_eq!(api.metadata_storage_label(), "memory");
        assert_eq!(api.pack_storage_label(), "local-file");

        match old_mode {
            Some(value) => std::env::set_var("BINDHUB_API_METADATA_MODE", value),
            None => std::env::remove_var("BINDHUB_API_METADATA_MODE"),
        }
        match old_database_url {
            Some(value) => std::env::set_var("BINDHUB_API_DATABASE_URL", value),
            None => std::env::remove_var("BINDHUB_API_DATABASE_URL"),
        }
    ...
    fn api_can_use_non_local_remote_storage() {
        let dir = tempfile::tempdir().expect("temp dir");
        let api = LocalBindhubApi::open_with_pack_storage(
            dir.path(),
    ...
    fn workos_session_association_uses_verified_boundary() {
        ...
        let authenticated = api
            .authenticate(&session.session_token, &session.device_id)
            .expect("stored Bindhub session token authenticates");
        let devices = api
    ...
    #[test]
    fn hosted_workos_exchange_requires_service_token_and_returns_bindhub_session() {
        let _guard = ENV_LOCK.lock().expect("env test lock");
        let old_service_token = std::env::var("BINDHUB_API_SERVICE_TOKEN").ok();
        std::env::set_var("BINDHUB_API_SERVICE_TOKEN", "service-secret");
        let dir = tempfile::tempdir().expect("temp dir");
        let api = LocalBindhubApi::open(dir.path()).expect("api opens");
        ...
        exchange_headers.insert(
            "x-bindhub-api-service-token".to_string(),
        ...
        assert!(session.session_token.starts_with("bindhub-workos-session-"));
        ...
        raw_workos_headers.insert("x-bindhub-device-id".to_string(), session.device_id.clone());
        assert!(matches!(
            route_request(
                &api,
                HttpRequest {
        ...
        let mut bindhub_headers = BTreeMap::new();
        bindhub_headers.insert(
            "authorization".to_string(),
            format!("Bearer {}", session.session_token),
        );
        bindhub_headers.insert("x-bindhub-device-id".to_string(), session.device_id);
        let devices_response = route_request(
            &api,
            HttpRequest {
                method: "GET".to_string(),
                path: "/v1/devices".to_string(),
                headers: bindhub_headers,
                body: Vec::new(),
            },
        )
        .expect("Bindhub session token can list devices");
        ...
        match old_service_token {
            Some(value) => std::env::set_var("BINDHUB_API_SERVICE_TOKEN", value),
            None => std::env::remove_var("BINDHUB_API_SERVICE_TOKEN"),
        }
    }
    ...
    fn cli_device_flow_requires_verified_browser_approval_before_poll_returns_session() {
        ...
        assert!(approved_session
            .session_token
            .starts_with("bindhub-workos-session-"));
        ...
        .expect("CLI Bindhub session can list devices");
        let devices_text = String::from_utf8(devices_response).expect("response is utf8");
        assert!(devices_text.starts_with("HTTP/1.1 200 OK"));
    ...
    fn ready_route_reports_service_health_without_auth() {
        ...
        assert!(text.contains("\"service\":\"bindhub-api\""));
