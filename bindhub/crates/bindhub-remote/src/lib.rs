//! Bindhub-hosted Loom remote implementation.
//!
//! Loom owns the remote trait and folder-state sync semantics. Bindhub owns the
...
pub const BINDHUB_HOSTED_REMOTE_KIND: &str = "bindhub";

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BindhubHostedRemoteConfig {
    api: Url,
...
impl BindhubHostedRemoteConfig {
    pub fn new(
        api: impl AsRef<str>,
        shared_folder_id: SharedFolderId,
    ...
    pub fn from_clone_url(value: &str) -> SyncResult<Self> {
        let url = Url::parse(value).map_err(|error| {
            SyncError::RemoteConfig(format!("bindhub remote URL is invalid: {error}"))
        })?;
        if url.scheme() != "bindhub" {
            return Err(SyncError::RemoteConfig(
                "bindhub remote URL must use the bindhub scheme".to_string(),
            ));
        }
        let shared_folder_id = url
            ...
            .ok_or_else(|| {
                SyncError::RemoteConfig(
                    "bindhub remote URL is missing a shared folder id".to_string(),
                )
            })
            .and_then(|value| SharedFolderId::new(value.to_string()).map_err(SyncError::Loom))?;
        let query = url.query_pairs().collect::<BTreeMap<_, _>>();
        let api = query.get("api").ok_or_else(|| {
            SyncError::RemoteConfig("bindhub remote URL is missing api".to_string())
        })?;
        let session = query.get("session").ok_or_else(|| {
            SyncError::RemoteConfig("bindhub remote URL is missing session".to_string())
        })?;
        let device = query.get("device").ok_or_else(|| {
            SyncError::RemoteConfig("bindhub remote URL is missing device".to_string())
        })?;
        Self::new(
            api.as_ref(),
            shared_folder_id,
    ...
    pub fn clone_url(&self) -> String {
        let mut url = Url::parse(&format!("bindhub://{}", self.shared_folder_id))
...
#[derive(Clone)]
pub struct BindhubHostedRemote {
    config: BindhubHostedRemoteConfig,
    agent: Agent,
}

impl fmt::Debug for BindhubHostedRemote {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("BindhubHostedRemote")
...
impl BindhubHostedRemote {
    pub fn new(config: BindhubHostedRemoteConfig) -> Self {
        Self {
            config,
            agent: Agent::new(),
        }
    ...
    pub fn config(&self) -> &BindhubHostedRemoteConfig {
        &self.config
    }
    ...
    fn request(&self, method: &str, url: &str) -> ureq::Request {
        self.agent
            ...
            .set("x-bindhub-device-id", &self.config.device_id)
    }
    ...
    fn call(&self, method: &str, url: &str, body: Option<&[u8]>) -> SyncResult<HttpResponse> {
        ...
        match response {
            ...
            Err(UreqError::Transport(error)) => Err(SyncError::RemoteTransport(format!(
                "bindhub hosted remote transport failed: {error}"
            ))),
        }
    }
}

impl LoomRemote for BindhubHostedRemote {
    ...
    fn get_cursor(&self, cursor_id: &str) -> SyncResult<Option<FolderRevisionId>> {
        ...
        match response.status {
            ...
            401 | 403 => Err(SyncError::RemoteAuth(format!(
                "bindhub hosted cursor read failed with HTTP status {}",
                response.status
            ))),
            404 => Ok(None),
            status => Err(SyncError::RemoteTransport(format!(
                "bindhub hosted cursor read failed with HTTP status {status}"
            ))),
        }
    }
    ...
    fn compare_and_set_cursor(
        ...
        match response.status {
            ...
            401 | 403 => Err(SyncError::RemoteAuth(format!(
                "bindhub hosted cursor update failed with HTTP status {}",
                response.status
            ))),
            status => Err(SyncError::RemoteTransport(format!(
                "bindhub hosted cursor update failed with HTTP status {status}"
            ))),
        }
    }
    ...
    fn put_pack(&self, pack: &LoomPack) -> SyncResult<()> {
        ...
        match response.status {
            200 | 201 => Ok(()),
            401 | 403 => Err(SyncError::RemoteAuth(format!(
                "bindhub hosted pack upload failed with HTTP status {}",
                response.status
            ))),
            status => Err(SyncError::RemoteTransport(format!(
                "bindhub hosted pack upload failed with HTTP status {status}"
            ))),
        }
    }
    ...
    fn get_pack(&self, revision_id: &FolderRevisionId) -> SyncResult<LoomPack> {
        ...
        match response.status {
            ...
            401 | 403 => Err(SyncError::RemoteAuth(format!(
                "bindhub hosted pack download failed with HTTP status {}",
                response.status
            ))),
            404 => Err(SyncError::MissingRemotePack(revision_id.clone())),
            status => Err(SyncError::RemoteTransport(format!(
                "bindhub hosted pack download failed with HTTP status {status}"
            ))),
        }
    }
    ...
    fn has_object(&self, object_id: &ObjectId) -> SyncResult<bool> {
        ...
        match response.status {
            200 => Ok(true),
            401 | 403 => Err(SyncError::RemoteAuth(format!(
                "bindhub hosted object availability check failed with HTTP status {}",
                response.status
            ))),
            404 => Ok(false),
            status => Err(SyncError::RemoteTransport(format!(
                "bindhub hosted object availability check failed with HTTP status {status}"
            ))),
        }
    }
    ...
    fn put_object(&self, object_id: &ObjectId, bytes: &[u8]) -> SyncResult<()> {
        ...
        match response.status {
            200 | 201 => Ok(()),
            401 | 403 => Err(SyncError::RemoteAuth(format!(
                "bindhub hosted object upload failed with HTTP status {}",
                response.status
            ))),
            409 => Err(SyncError::RemoteTransport(
                "bindhub hosted object upload conflicted with different bytes".to_string(),
            )),
            status => Err(SyncError::RemoteTransport(format!(
                "bindhub hosted object upload failed with HTTP status {status}"
            ))),
        }
    }
    ...
    fn get_object(&self, object_id: &ObjectId) -> SyncResult<Vec<u8>> {
        ...
        match response.status {
            ...
            401 | 403 => Err(SyncError::RemoteAuth(format!(
                "bindhub hosted object download failed with HTTP status {}",
                response.status
            ))),
            404 => Err(SyncError::RemoteTransport(format!(
                "bindhub hosted object {object_id} was not found"
            ))),
            status => Err(SyncError::RemoteTransport(format!(
                "bindhub hosted object download failed with HTTP status {status}"
            ))),
        }
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BindhubProvisionedRemote {
    pub config: BindhubHostedRemoteConfig,
...
pub fn provision_bindhub_hosted_remote(
    api: impl AsRef<str>,
    shared_folder_id: &SharedFolderId,
    display_name: &str,
) -> SyncResult<BindhubProvisionedRemote> {
    let api = parse_http_url(api.as_ref())?;
    let agent = Agent::new();
    ...
    let login = match login {
        ...
        Err(UreqError::Transport(error)) => {
            return Err(SyncError::RemoteTransport(format!(
                "bindhub local session request failed: {error}"
            )))
        }
    };
    if login.status != 200 {
        return Err(SyncError::RemoteAuth(format!(
            "bindhub local session request failed with HTTP status {}",
            login.status
        )));
    }
    ...
    let folder = match folder {
        ...
        Err(UreqError::Transport(error)) => {
            return Err(SyncError::RemoteTransport(format!(
                "bindhub shared folder registration failed: {error}"
            )))
        }
    };
    if folder.status != 200 {
        return Err(SyncError::RemoteAuth(format!(
            "bindhub shared folder registration failed with HTTP status {}",
            folder.status
        )));
    }

    Ok(BindhubProvisionedRemote {
        config: BindhubHostedRemoteConfig::new(
            api.as_str(),
...
mod tests {
    ...
    fn bindhub_hosted_remote_moves_pack_and_cursor_through_api() {
        ...
        let remote = BindhubHostedRemote::new(provisioned.config.clone());
        ...
        assert!(provisioned.config.clone_url().starts_with("bindhub://"));
        assert_eq!(
            BindhubHostedRemoteConfig::from_clone_url(&provisioned.config.clone_url())
    ...
    fn bindhub_hosted_remote_reports_cursor_conflict() {
        ...
        let remote = BindhubHostedRemote::new(provisioned.config);
    ...
    fn bindhub_hosted_remote_refuses_unauthorized_pack_and_object_access() {
        ...
        let unauthorized = BindhubHostedRemote::new(
            BindhubHostedRemoteConfig::new(
                api.base_url(),
