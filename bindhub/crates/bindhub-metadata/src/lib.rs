...
const MOCK_ACCOUNT_HEADER: &str = "x-bindhub-mock-account-id";
const MOCK_DEVICE_HEADER: &str = "x-bindhub-mock-device-id";
...
pub struct ReadyResponse {
    ...
    pub object_access_default_store_configured: bool,
}
...
pub struct ManagedObjectAccessBrokerConfig {
    ...
    pub default_provider_kind: Option<ManagedObjectProviderKind>,
...
impl ManagedObjectAccessBrokerConfig {
    ...
    pub fn with_default_object_store(
        ...
        self.default_provider_kind = Some(provider_kind);
        self.default_endpoint = Some(sanitize_object_endpoint(endpoint.as_ref())?);
        self.default_bucket = Some(validate_object_bucket(bucket.as_ref())?);
        self.default_region = Some(public_metadata_identifier(region.as_ref(), "region")?);
        Ok(self)
    }
    ...
    pub fn has_default_object_store(&self) -> bool {
        self.default_provider_kind.is_some()
            && self.default_endpoint.is_some()
            && self.default_bucket.is_some()
            && self.default_region.is_some()
    }
}
...
VALUES (1, 'bindhub-metadata-postgres-v1')
    ...
    S: MetadataStore,
{
    Json(HealthResponse {
        status: "ok".to_string(),
        service: "bindhub-metadata".to_string(),
    ...
    S: MetadataStore,
{
    Json(ReadyResponse {
        ...
        object_access_default_store_configured: state
            .config
            .object_access_broker
            .has_default_object_store(),
    })
}
...
where
    S: MetadataStore,
{
    let now_unix = bindhub_auth::now_unix_seconds();
    ...
    let provider_issuer = "bindhub-alpha";
    ...
    S: MetadataStore,
{
    ...
    let session =
        authenticate_account_session(&*store, &raw_token, bindhub_auth::now_unix_seconds())?;
    let stored = store
    ...
    S: MetadataStore,
{
    ...
    Ok(Response::builder()
        ...
        .header("x-bindhub-object-size", bytes.len().to_string())
    ...
    S: MetadataStore,
{
    ...
    Ok(Response::builder()
        .status(StatusCode::OK)
        .header("x-bindhub-object-size", metadata.size_bytes.to_string())
        .body(Body::empty())
        .map_err(|error| MetadataError::InvalidRequest(error.to_string()))?)
}
...
pub fn managed_object_access_grant_for_account_session<S: MetadataStore + ?Sized>(
    ...
    let record = match store.managed_object_credential_lease(
        ...
        Some(record) => record,
        None => {
            match store.managed_object_credential_lease(&session.account_id, None, &lease_id)? {
                Some(record) => record,
                None => default_managed_object_credential_lease(
                    session,
                    &project_id,
                    &lease_id,
                    broker_config,
...
fn default_managed_object_credential_lease(
    session: &AuthenticatedAccountSession,
    project_id: &str,
    lease_id: &str,
    broker_config: &ManagedObjectAccessBrokerConfig,
    ...
    let Some(provider_kind) = broker_config.default_provider_kind else {
        return Ok(None);
    };
    let Some(endpoint) = broker_config.default_endpoint.clone() else {
        return Ok(None);
    };
    let Some(bucket) = broker_config.default_bucket.clone() else {
        return Ok(None);
    };
    let Some(region) = broker_config.default_region.clone() else {
        return Ok(None);
    };

    Ok(Some(ManagedObjectCredentialLeaseRecord {
        account_id: session.account_id.clone(),
        ...
        bucket,
        region,
        prefix: Some(project_scoped_object_prefix(
            &session.account_id,
            project_id,
        )?),
        credential_reference: "server-managed-object-store".to_string(),
        ...
        issued_at: format!("unix:{now_unix}"),
...
fn auth_error(error: bindhub_auth::AuthError) -> MetadataError {
    MetadataError::InvalidAccountSession(error.to_string())
}

fn auth_proof_error(error: bindhub_auth::AuthError) -> MetadataError {
...
mod tests {
    ...
    const PROJECT: &str = "project-bindhub";
    ...
    fn postgres_store_matches_sqlite_core_semantics_when_configured() {
        let Some(mut postgres_core) = PostgresTestContext::open() else {
            eprintln!(
                "skipping Postgres metadata parity test; BINDHUB_TEST_POSTGRES_URL is not set"
            );
            return;
        };
    ...
    fn in_memory_account_session_requires_matching_provider_tuple() {
        let raw_token = "raw-hosted-session-token";
        let proof = account_proof();
        let mut session = bindhub_auth::create_account_session(
            &proof,
            raw_token,
            "2026-06-18T10:01:00Z",
            101,
    ...
    fn in_memory_account_session_rejects_duplicate_token_hash() {
        let raw_token = "raw-hosted-session-token";
        let proof = account_proof();
        let first = bindhub_auth::create_account_session(
            &proof,
            raw_token,
            "2026-06-18T10:01:00Z",
            101,
        ...
        let second = bindhub_auth::create_account_session(
            &proof,
            raw_token,
    ...
    async fn latest_snapshot_route_returns_newest_project_record() {
        ...
        let response = app
            .oneshot(json_request::<()>(
                Method::GET,
                "/v1/projects/project-bindhub/snapshots/latest",
                &(),
                true,
            ))
            .await
    ...
    fn endpoint_validation_rejects_unsafe_urls_and_returns_sanitized_output() {
        let safe = MetadataServiceConfig {
            endpoint: "https://metadata.example:8443/bindhub".to_string(),
            auth_mode: MetadataAuthMode::MockDevHeaders,
        }
        .validate()
        .expect("safe endpoint validates");
        assert_eq!(safe.endpoint, "https://metadata.example:8443/bindhub");

        for (endpoint, expected) in [
            ("", "metadata endpoint must not be empty"),
            (
                "metadata.example/bindhub",
    ...
    async fn handlers_require_mock_dev_identity_and_return_cursor_conflicts() {
        ...
        let missing_auth = app
            ...
            .oneshot(json_request(
                Method::PUT,
                "/v1/cursors/project-bindhub/device-laptop",
                &UpdateCursorRequest {
                    account_id: ACCOUNT.to_string(),
                    device_id: DEVICE.to_string(),
                    project_id: PROJECT.to_string(),
    ...
    async fn handlers_cover_health_registration_publish_and_fetch() {
        ...
        let publish = app
            ...
            .oneshot(json_request(
                Method::PUT,
                "/v1/projects/project-bindhub/snapshots",
                &publish_request(),
                true,
            ))
            .await
        ...
        let fetched = app
            .oneshot(
                Request::builder()
                    .method(Method::GET)
                    .uri("/v1/projects/project-bindhub/snapshots/snapshot-a")
    ...
    fn managed_object_access_grants_enforce_shared_bucket_prefix_scope() {
        ...
        assert_eq!(
            grant.prefix,
            "accounts/account-alpha/projects/project-bindhub"
        );
        assert_eq!(
        ...
        assert!(!format!("{grant:?}").contains("BINDHUB_R2_SECRET_ACCESS_KEY"));
        ...
        let other_session = bindhub_auth::create_account_session(
            &other_proof,
            other_raw_token,
            "2026-06-18T10:01:00Z",
            101,
    ...
    fn managed_object_access_rejects_prefix_escape_and_wildcard_scope() {
        ...
        for prefix in [
            ...
            "accounts/account-alpha/projects/project-bindhub/../project-other",
            "*",
        ] {
            assert!(
    ...
    async fn object_access_handler_requires_session_and_returns_scoped_grant() {
        ...
        let mock_headers = router
            ...
            .oneshot(json_request(
                Method::POST,
                "/v1/projects/project-bindhub/object-access/lease-alpha",
                &request,
                true,
            ))
            .await
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let body = response_text(response).await;
        assert!(body.contains("\"prefix\":\"accounts/account-alpha/projects/project-bindhub\""));
        assert!(body.contains("\"credential_delivery\":\"server-mediated-broker\""));
        assert!(!body.contains("BINDHUB_R2_SECRET_ACCESS_KEY"));
    ...
    async fn hosted_object_transfer_uses_server_storage_without_client_bucket_keys() {
        ...
        .with_default_object_store(
            ManagedObjectProviderKind::MinioCompatible,
            "http://localhost",
            "bindhub-local",
            "local",
        )
        ...
        tokio::task::spawn_blocking(move || {
            let provider = bindhub_sync::HostedObjectTransferProvider::new(
                bindhub_sync::HostedObjectTransferConfig::new(
                    format!("http://{addr}"),
                    PROJECT,
                    "bindhub-managed",
    ...
    async fn hosted_object_upload_accepts_blobs_above_axum_default_limit() {
        ...
        let response = router
            .oneshot(session_binary_request(
                Method::PUT,
                "/v1/projects/project-bindhub/object-access/lease-alpha/object?key=encrypted/blobs/b3/large/object",
                &body,
                raw_token,
            ))
            .await
    ...
    async fn hosted_object_transfer_enforces_scope_capabilities_and_lease_state() {
        ...
        let missing_write = router
            ...
            .oneshot(session_binary_request(
                Method::PUT,
                "/v1/projects/project-bindhub/object-access/lease-alpha/object?key=encrypted/blobs/test",
                b"ciphertext",
                raw_token,
            ))
            .await
        ...
        let escaped_key = router
            ...
            .oneshot(session_binary_request(
                Method::PUT,
                "/v1/projects/project-bindhub/object-access/lease-alpha/object?key=accounts/account-other/projects/project-bindhub/blob",
                b"ciphertext",
                raw_token,
            ))
            .await
        ...
        let unsafe_key = router
            ...
            .oneshot(session_binary_request(
                Method::PUT,
                "/v1/projects/project-bindhub/object-access/lease-alpha/object?key=encrypted/blobs/bad%0Aline",
                b"ciphertext",
                raw_token,
            ))
            .await
        ...
        other_store
            .upsert_account_session(
                bindhub_auth::create_account_session(
                    &other_proof,
                    other_raw_token,
                    "2026-06-18T10:01:00Z",
                    101,
    ...
    impl PostgresTestContext {
        fn open() -> Option<Self> {
            let admin_url = std::env::var("BINDHUB_TEST_POSTGRES_URL").ok()?;
            let unique_nanos = std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("system clock is after unix epoch")
                .as_nanos();
            let schema = format!("bindhub_test_{}_{}", std::process::id(), unique_nanos);
            let mut admin =
    ...
    fn managed_lease_request() -> ManagedObjectCredentialLeaseRequest {
        ManagedObjectCredentialLeaseRequest {
            ...
            bucket: "bindhub-alpha".to_string(),
            region: "auto".to_string(),
            prefix: Some("accounts/account-alpha/projects/project-bindhub".to_string()),
    ...
    fn project_request() -> UpsertProjectRequest {
        UpsertProjectRequest {
            ...
            root_hint: "~/Code/bindhub".to_string(),
            project_kind: "rust".to_string(),
