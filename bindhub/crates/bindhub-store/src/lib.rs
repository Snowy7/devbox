//! SQLite-backed local metadata boundary for Bindhub.
...
use bindhub_conflict::{ConflictSummary, PathComparisonState};
use bindhub_core::{BlobId, DomainIdError, ManifestEntryKind, PolicyDecision, ProjectId};
...
mod tests {
    ...
    fn file_backed_store_persists_schema_version() {
        ...
        let path = dir.path().join("bindhub.sqlite3");
    ...
    fn mock_auth_session_is_idempotent() {
        ...
        let first = bindhub_auth::mock_login(&view, "2026-06-18T10:00:00Z");
        let second = bindhub_auth::mock_login(&view, "2026-06-18T10:01:00Z");
    ...
    fn production_account_proof_and_session_round_trip_without_raw_token_persistence() {
...

        {
            ...
            bindhub_auth::validate_account_session(&loaded_session, raw_token, 102)
    ...
    fn production_account_session_revocation_persists() {
        ...
        store
            .upsert_account_session(&session)
            .expect("session persists");
        let revoked = bindhub_auth::revoke_account_session(&session, "2026-06-18T10:02:00Z")
        ...
        assert!(matches!(
            bindhub_auth::validate_account_session(&loaded, raw_token, 102),
            Err(bindhub_auth::AuthError::AccountSessionRevoked { .. })
        ));
    }
    ...
    fn pairing_lifecycle_approves_many_devices_and_creates_envelopes() {
        ...
        for (index, name) in ["Laptop", "Build box", "Travel kit"].iter().enumerate() {
            ...
            let approval = bindhub_auth::approve_pairing_invitation(
                &view,
                &loaded,
                &draft.token,
                name,
            ...
            let opened = bindhub_auth::open_key_envelope(
                &envelope,
                &approval.device.device_key_hex,
                &approval.device.device_id,
            )
    ...
    fn receiver_pairing_completion_installs_local_sync_key_without_source_db() {
        ...
        let join =
            bindhub_auth::create_pairing_join_request(&draft.token, &receiver_identity.device_id)
                .expect("join creates");
        assert!(!join
            .expose_for_cli()
    ...
    fn pairing_invitation_can_only_be_persisted_once() {
        ...
        let loaded = store
            ...
            .expect("invitation reads")
            .expect("invitation exists");
        let first = bindhub_auth::approve_pairing_invitation(
            &view,
            &loaded,
            &draft.token,
            "Laptop",
        ...
        let second = bindhub_auth::approve_pairing_invitation(
            &view,
            &loaded,
            &draft.token,
            "Second laptop",
    ...
    fn malformed_expired_and_reused_invitations_fail_before_store_update() {
        ...
        let identity = store
            ...
            .expect("identity initializes");
        let view = local_identity_view(&identity);
        let draft = bindhub_auth::create_pairing_invitation(&view, "2026-06-18T10:00:00Z", 100, 1)
            .expect("invitation creates");
        store
            .insert_pairing_invitation(&draft.invitation)
            .expect("invitation persists");
        assert!(bindhub_auth::PairingInvitationToken::parse("bad-token").is_err());

        let loaded = store
            .pairing_invitation(&draft.invitation.id)
            .expect("invitation reads")
            .expect("invitation exists");
        let expired = bindhub_auth::approve_pairing_invitation(
            &view,
            &loaded,
            &draft.token,
            "Laptop",
        ...
        assert!(matches!(
            expired,
            Err(bindhub_auth::AuthError::InvitationExpired { .. })
        ));
        assert_eq!(
            store
                .pairing_invitation(&draft.invitation.id)
    ...
    fn production_recovery_grant_revocation_is_idempotent_and_sanitized() {
...

        {
            ...
            let rejected = bindhub_auth::create_recovery_grant(
                &identity.account_id,
                &approval.device.device_id,
                raw_recovery_secret,
                "laptop recovery",
            ...
            let grant = bindhub_auth::create_recovery_grant(
                &identity.account_id,
                &approval.device.device_id,
            ...
            let consumable = bindhub_auth::create_recovery_grant(
                &identity.account_id,
                &approval.device.device_id,
    ...
    fn production_device_rotation_intent_bumps_key_envelope_generation() {
        ...
        let intent =
            bindhub_auth::create_device_rotation_intent(bindhub_auth::DeviceRotationIntentInput {
                account_id: &identity.account_id,
    ...
    fn project_and_snapshot_metadata_round_trip() {
        ...
        assert_eq!(project.root_path, "/workspace/bindhub");
    ...
    fn draft_snapshot_persistence_round_trips_manifest_metadata() {
        ...
        assert_eq!(persisted.project.root_path, "/workspace/bindhub");
    ...
    fn local_identity_view(identity: &LocalIdentityRecord) -> bindhub_auth::LocalIdentityView {
