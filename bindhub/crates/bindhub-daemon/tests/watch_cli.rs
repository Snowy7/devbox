...
use bindhub_snapshot::SnapshotManifestBuilder;
use bindhub_store::{
...
use bindhub_sync::LocalFilesystemBlobProvider;
...
fn watch_once_rejects_in_project_cache_and_db_without_leftovers() {
    ...
    let rejected_cache = run_bindhub_daemon([
        "watch",
        "--db",
        fixture.db_path(),
        "--cache",
    ...
    let rejected_db = run_bindhub_daemon([
        "watch",
        "--db",
...
fn sync_pull_discovers_latest_snapshot_and_materializes_to_empty_target() {
    ...
    let output = run_bindhub_daemon_vec(vec![
        "sync".to_string(),
        "--db".to_string(),
...
fn sync_pull_refuses_divergent_cursor_before_downloading_blobs() {
    ...
    let blocked = run_bindhub_daemon_vec(vec![
        "sync".to_string(),
        "--db".to_string(),
        path_string(&fixture.receiver_db),
        "--cache".to_string(),
...
fn sync_s3_live_mode_requires_object_access_and_env_names() {
    let missing_object_access = run_bindhub_daemon([
        "sync",
        "--db",
        "bindhub.sqlite3",
        "--cache",
    ...
    let raw_env_name = run_bindhub_daemon([
        "sync",
        "--db",
        "bindhub.sqlite3",
        "--cache",
    ...
    assert!(!stderr(&raw_env_name).contains("BINDHUB_R2_SECRET_ACCESS_KEY"));
}
...
fn live_sync_script_documents_hosted_api_without_shared_metadata_db() {
    ...
    let script = fs::read_to_string(repo.join("scripts/bindhub-live-sync-alpha.sh"))
        .expect("live sync script reads");

    assert!(env_example.contains("BINDHUB_METADATA_PROJECT="));
    assert!(script.contains("BINDHUB_METADATA_PROJECT:?set BINDHUB_METADATA_PROJECT"));
    assert!(script.contains("--metadata-mode hosted-api"));
    assert!(script.contains("--metadata-api"));
    assert!(script.contains("--metadata-session-token-env BINDHUB_SESSION_TOKEN"));
    assert!(!script.contains("set BINDHUB_METADATA_DB for hosted live sync metadata"));
}
...
fn sync_hosted_object_transfer_push_and_materialize_without_client_r2_keys() {
    ...
    let push = run_bindhub_daemon_vec_with_env(
        fixture.hosted_sync_args(&fixture.source_db, &fixture.source_cache, &server.url, true),
        &[("BINDHUB_TEST_SESSION_TOKEN", session_token)],
    );
    assert_success(&push);
    ...
    assert!(!push_stdout.contains("BINDHUB_R2_ACCESS_KEY_ID"));
    assert!(!push_stdout.contains("BINDHUB_R2_SECRET_ACCESS_KEY"));
    assert!(fixture
        .hosted_object_root
        .join("objects")
        .join("accounts")
    ...
    fixture.pair_receiver_with_source();
    fs::create_dir_all(&fixture.target).expect("target creates");
    let pull = run_bindhub_daemon_vec_with_env(
...
impl LiveFixture {
    ...
    fn init_identity(&self, db_path: &Path, name: &str) -> bindhub_store::LocalIdentityRecord {
    ...
    fn hosted_sync_args(
        ...
        let mut args = vec![
            ...
            "BINDHUB_TEST_SESSION_TOKEN".to_string(),
            "--metadata-mode".to_string(),
...
fn run_bindhub_daemon<const N: usize>(args: [&str; N]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_bindhub-daemon"))
        .args(args)
        .output()
        .expect("bindhub-daemon command runs")
}

fn run_bindhub_daemon_vec(args: Vec<String>) -> Output {
    Command::new(env!("CARGO_BIN_EXE_bindhub-daemon"))
        .args(args)
        .output()
        .expect("bindhub-daemon command runs")
...
fn run_bindhub_daemon_vec_with_env(args: Vec<String>, envs: &[(&str, &str)]) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_bindhub-daemon"));
    command.args(args);
    for (name, value) in envs {
        command.env(name, value);
    }
    command
        .env_remove("BINDHUB_R2_ACCESS_KEY_ID")
        .env_remove("BINDHUB_R2_SECRET_ACCESS_KEY")
...
fn identity_view(identity: &bindhub_store::LocalIdentityRecord) -> LocalIdentityView {
    LocalIdentityView {
