...
fn main() -> ExitCode {
    ...
    match args.first().map(String::as_str) {
        Some("--version") | Some("-V") | Some("version") => {
            println!("bindhub-daemon {VERSION}");
            ExitCode::SUCCESS
        }
        ...
        Some(command) => {
            eprintln!("bindhub-daemon: unknown command '{command}'");
            eprintln!("Run 'bindhub-daemon --help' for usage.");
            ExitCode::from(2)
        }
    }
}
...
fn completed_identity(db_path: &Path) -> Result<bindhub_store::LocalIdentityRecord, String> {
    ...
    store
        ...
        .map_err(|error| error.to_string())?
        .ok_or_else(|| {
            "local identity is not initialized; run bindhub init --db <DB_PATH>".to_string()
        })
}
...
fn discover_pull_snapshot(
    ...
    let latest = match args.metadata.mode {
        ...
        SyncMetadataMode::HostedApi => {
            let mut metadata_client = open_hosted_metadata_client(&args.metadata)?;
            bindhub_materialize::HostedMetadataClient::latest_snapshot(
                &mut metadata_client,
                account_id,
                project_id,
            )
...
fn import_for_live_sync(
    ...
    project_id: &str,
    snapshot_id: &str,
) -> Result<bindhub_materialize::ImportedSnapshotBundle, String> {
...
fn materialize_for_live_sync(
    ...
    snapshot_id: &str,
    target: &Path,
) -> Result<bindhub_materialize::MaterializationOutcome, String> {
...
fn print_help() {
    println!("bindhub-daemon {VERSION}");
    println!();
    println!("Usage: bindhub-daemon <COMMAND>");
    println!();
    println!("Commands:");
    ...
    println!("Watch usage:");
    println!(
        "  bindhub-daemon watch --db <DB_PATH> --cache <CACHE_ROOT> <PROJECT_ROOT> [--once] [--debounce-ms <MS>] [--exit-after-idle-ms <MS>] [--max-scans <N>]"
    );
    println!();
    println!("Sync usage:");
    println!(
        "  bindhub-daemon sync --db <DB_PATH> --cache <CACHE_ROOT> --remote <REMOTE_DIR> [--metadata-mode mock-dev-sqlite --metadata-db <METADATA_DB>] [--push|--pull|--two-way] <PROJECT_ROOT> [--once] [--debounce-ms <MS>] [--exit-after-idle-ms <MS>] [--max-cycles <N>]"
    );
    println!(
        "  bindhub-daemon sync --pull --db <DB_PATH> --cache <CACHE_ROOT> --remote <REMOTE_DIR> --metadata-mode mock-dev-sqlite --metadata-db <METADATA_DB> --metadata-account <ACCOUNT_ID> --metadata-project <PROJECT_ID> [--to <TARGET_DIR> --apply] <PROJECT_ROOT> [--once]"
    );
    println!(
        "  bindhub-daemon sync --remote-kind hosted --object-access-api <URL> --object-access-lease <LEASE_ID> --metadata-mode hosted-api --metadata-api <URL> --metadata-project <PROJECT_ID> [--metadata-session-token-env BINDHUB_SESSION_TOKEN] [--push|--pull|--two-way] <PROJECT_ROOT> [--once]"
    );
    println!(
