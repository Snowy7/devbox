...
fn help_separates_product_commands_from_alpha_compatibility() {
    let output = run_bindhub_with_env([], ["--help"]);
    ...
    for args in [
        ...
        assert!(stdout.contains("bindhub keeps folders continuous across machines."));
...
fn product_login_share_clone_status_pause_resume_and_unlink_flow() {
    ...
    assert!(login_stdout.contains("Logged in to Bindhub"));
    assert!(login_stdout.contains("Token: not printed"));
    assert!(!login_stdout.contains("bindhub-local-session"));

    let share = fixture.bindhub(["share", path_str(&fixture.source), "--no-background-sync"]);
    ...
    let status = fixture.bindhub(["status"]);
    ...
    let doctor = fixture.bindhub(["doctor"]);
    ...
    let pause = fixture.bindhub(["pause", path_str(&fixture.target)]);
    assert_success(&pause);
    assert!(stdout(&pause).contains("Files: left untouched"));
    assert!(fixture.target.join("README.md").exists());

    let resume = fixture.bindhub(["resume", path_str(&fixture.target), "--no-background-sync"]);
    assert_success(&resume);
    assert!(stdout(&resume).contains("Resumed sync for source"));

    let unlink = fixture.bindhub(["unlink", path_str(&fixture.target)]);
...
fn product_sparse_folder_commands_are_intent_based_and_safe() {
    ...
    assert_success(&fixture.bindhub([
        "login",
        "--local-dev-direct",
        "--api",
        &fixture.api.base_url(),
    ...
    assert_success(&fixture.bindhub(["share", path_str(&fixture.source), "--no-background-sync"]));
    ...
    let sparse_status = fixture.bindhub(["status", path_str(&fixture.target)]);
    ...
    let hydrate = fixture.bindhub(["hydrate", path_str(&fixture.target.join("README.md"))]);
    ...
    let warm = fixture.bindhub(["warm", path_str(&fixture.target), "--max-bytes", "20"]);
    assert_success(&warm);
    ...
    let keep = fixture.bindhub(["keep", path_str(&fixture.target.join("README.md"))]);
    assert_success(&keep);
    ...
    let free_space = fixture.bindhub(["free-space", path_str(&fixture.target), "--max-bytes", "0"]);
    ...
    let status = fixture.bindhub(["status", path_str(&fixture.target)]);
    assert_success(&status);
    let status_stdout = stdout(&status);
...
fn product_free_space_refuses_without_backed_up_copy() {
    ...
    let free_space = fixture.bindhub(["free-space", path_str(&fixture.source), "--max-bytes", "0"]);
...
fn unauthenticated_share_and_clone_fail_without_touching_files() {
    ...
    let clone = fixture.bindhub(["clone", "source", path_str(&fixture.target)]);
...
fn local_dev_direct_login_refuses_non_loopback_api_by_default() {
    ...
    for api in ["https://api.bindhub.example", "https://127.evil.example"] {
        let login = run_bindhub_with_env(
            [("BINDHUB_CONFIG_DIR", path_str(&config))],
            [
                "login",
                "--local-dev-direct",
                "--api",
        ...
        assert!(!stderr.contains("could not reach Bindhub"));
    }
}
...
fn invalid_session_resume_asks_user_to_login_without_internal_terms() {
    ...
    let resume = fixture.bindhub(["resume", path_str(&fixture.source), "--no-background-sync"]);
    assert_failure(&resume);
    let combined = format!("{}\n{}", stdout(&resume), stderr(&resume));
    assert!(combined.contains("run bindhub login again"));
    assert_product_output_is_clean(&combined);
}
...
fn another_account_cannot_clone_a_protected_shared_folder() {
    let dir = tempfile::tempdir().expect("temp dir");
    let api = bindhub_api::spawn_local_test_server(dir.path().join("api")).expect("api starts");
    ...
    assert_success(&run_bindhub_with_env(
        [("BINDHUB_CONFIG_DIR", path_str(&alice_config))],
        [
            "login",
            "--local-dev-direct",
            "--api",
    ...
    assert_success(&run_bindhub_with_env(
        [("BINDHUB_CONFIG_DIR", path_str(&bob_config))],
        [
            "login",
            "--local-dev-direct",
            "--api",
    ...
    let bob_clone = run_bindhub_with_env(
        [("BINDHUB_CONFIG_DIR", path_str(&bob_config))],
...
struct ProductCliFixture {
    ...
    api: bindhub_api::LocalApiServer,
    api_root: PathBuf,
    config: PathBuf,
    source: PathBuf,
    target: PathBuf,
...
impl ProductCliFixture {
    fn new(name: &str) -> Self {
        ...
        let api = bindhub_api::spawn_local_test_server(&api_root).expect("api starts");
        let source = dir.path().join("source");
        let target = dir.path().join("target");
        fs::create_dir_all(&source).expect("source creates");
        Self {
    ...
    fn bindhub<const N: usize>(&self, args: [&str; N]) -> Output {
        run_bindhub_with_env([("BINDHUB_CONFIG_DIR", path_str(&self.config))], args)
    }
...
fn run_bindhub_with_env<const E: usize, const N: usize>(
    envs: [(&str, &str); E],
    args: [&str; N],
) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_bindhub"));
    command.args(args);
    for (name, value) in envs {
        command.env(name, value);
    }
    command.output().expect("bindhub command runs")
}
...
fn assert_product_output_is_clean(output: &str) {
    for hidden_word in ["pack", "cursor", "remote", "bindhub://", "loom"] {
        assert!(
