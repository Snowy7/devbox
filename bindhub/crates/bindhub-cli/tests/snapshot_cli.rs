...
fn snapshot_create_list_and_show_smoke() {
    ...
    let list = run_bindhub(["snapshot", "list", "--db", fixture.db_path()]);
    assert_success(&list);
    ...
    let show = run_bindhub(["snapshot", "show", "--db", fixture.db_path(), snapshot_id]);
    assert_success(&show);
    ...
    let restore_dry_run = run_bindhub([
        "snapshot",
        "restore",
        "--db",
        fixture.db_path(),
...
fn init_is_idempotent_and_devices_list_current_local_device() {
    ...
    let devices = run_bindhub(["devices", "list", "--db", fixture.db_path()]);
...
fn auth_device_pairing_and_cursor_smoke_use_local_mock_metadata() {
    ...
    let login = run_bindhub(["auth", "mock-login", "--db", fixture.db_path()]);
...
fn sync_upload_and_download_encrypts_remote_object_bytes() {
    ...
    assert_success(&run_bindhub(["init", "--db", fixture.db_path()]));
    let create = run_bindhub([
        "snapshot",
        "--db",
        fixture.db_path(),
...
fn sync_remote_check_s3_validate_only_redacts_credentials() {
    ...
    assert!(output.contains("Remote bucket: bindhub-alpha"));
    assert!(output.contains("Remote region: auto"));
    assert!(output.contains("Remote prefix: accounts/acct/projects"));
    assert!(output.contains("Credential access key env: BINDHUB_R2_ACCESS_KEY_ID"));
    assert!(output.contains("Credential secret key env: BINDHUB_R2_SECRET_ACCESS_KEY"));
...
fn sync_remote_check_hosted_validate_only_uses_no_bucket_credentials() {
    ...
    assert!(output.contains("Remote project id: project-bindhub"));
    assert!(output.contains("Object access lease: lease-alpha"));
    assert!(output.contains("Session token env: BINDHUB_SESSION_TOKEN"));
    assert!(output.contains("Client bucket credentials: not used"));
    assert!(output.contains("Credentials redacted: true"));
    assert!(!output.contains("BINDHUB_R2_ACCESS_KEY_ID"));
    assert!(!output.contains("BINDHUB_R2_SECRET_ACCESS_KEY"));
}
...
fn changes_scan_and_list_smoke_are_stable_and_idempotent() {
    ...
    let project_id = first_scan_stdout
        ...
        .to_string();
    let list = run_bindhub(["changes", "list", "--db", fixture.db_path()]);
    assert_success(&list);
    let list_stdout = stdout(&list);
...
fn conflicts_compare_list_show_and_resolve_divergent_snapshots() {
    ...
    let second_compare = run_bindhub([
        "conflicts",
        "compare",
        "--db",
        fixture.db_path(),
    ...
    let list = run_bindhub(["conflicts", "list", "--db", fixture.db_path()]);
    assert_success(&list);
    let list_stdout = stdout(&list);
    assert!(list_stdout.contains(
    ...
    let show = run_bindhub(["conflicts", "show", "--db", fixture.db_path(), &conflict_id]);
    assert_success(&show);
    let show_stdout = stdout(&show);
...
fn changes_scan_rejects_in_project_cache_and_db_without_leftovers() {
    let fixture = SnapshotCliFixture::new();
    fixture.write("README.md", "hello\n");
    let in_project_cache = fixture.project.join(".bindhub-cache");
    let in_project_db = fixture.project.join("bindhub.sqlite3");

    let rejected_cache = run_bindhub([
        "changes",
        "scan",
        "--db",
        fixture.db_path(),
...
fn snapshot_create_rejects_in_project_db_without_metadata_leftovers() {
    ...
    assert!(!show_stdout.contains("bindhub.sqlite3\tfile\tinclude"));
}
...
impl SnapshotCliFixture {
    fn new() -> Self {
        ...
        let db = dir.path().join("bindhub.sqlite3");
...
fn run_bindhub<const N: usize>(args: [&str; N]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_bindhub"))
        .args(args)
        .output()
        .expect("bindhub command runs")
}
...
fn insert_manual_snapshot(
    ...
    conn.execute(
        ...
        VALUES (?1, '/workspace/bindhub', 'local', 'bindhub', ?2)
        "#,
