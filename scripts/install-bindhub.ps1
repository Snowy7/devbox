...
function Get-ReleaseTag {
    ...
    if (-not $release) {
        throw "No Bindhub releases found for $Repo"
    }
...
function GithubHeaders {
    ...
    $headers = @{
        "User-Agent" = "Bindhub-installer"
        "Accept" = if ($Download) { "application/octet-stream" } else { "application/vnd.github+json" }
    }
    $token = if ($env:BINDHUB_GITHUB_TOKEN) {
        $env:BINDHUB_GITHUB_TOKEN
    } else {
        $env:GITHUB_TOKEN
    }
    if ($token) {
...
try {
    ...
    $PackageDir = Join-Path $TempDir "bindhub-$Tag-$Target"
    ...
    Write-Host "Bindhub $Tag installed to $InstallDir"
    if ($NoPath) {
        Write-Host "PATH was not changed because -NoPath was supplied."
    } else {
        Write-Host "Open a new terminal, then run: bindhub --help"
    }
