...
fn print_sync_usage() {
    ...
    eprintln!(
        "  Add --metadata-mode hosted-api --metadata-api <URL> [--metadata-session-token-env DEVBOX_SESSION_TOKEN] for external hosted metadata registration, latest discovery, and cursor CAS."
    );
    eprintln!(
