    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--help" | "-h" => {
                ...
                println!("Object broker env: DEVBOX_R2_ENDPOINT, DEVBOX_R2_BUCKET, DEVBOX_R2_ACCESS_KEY_ID, DEVBOX_R2_SECRET_ACCESS_KEY, optional DEVBOX_R2_SESSION_TOKEN. Local/dev can use DEVBOX_OBJECT_LOCAL_ROOT.");
                return Ok(());
            }
...
fn object_access_broker_config_from_env(
        ...
        if !root.trim().is_empty() {
            return Ok(ManagedObjectAccessBrokerConfig::server_managed_local(root)?
                .with_default_object_store(
                    ...
                    optional_env_value("DEVBOX_OBJECT_LOCAL_ENDPOINT")
                        .unwrap_or_else(|| "http://localhost".to_string()),
                    optional_env_value("DEVBOX_OBJECT_LOCAL_BUCKET")
                        .unwrap_or_else(|| "devbox-local".to_string()),
                    optional_env_value("DEVBOX_OBJECT_LOCAL_REGION")
    ...
    if env_value_present(&access_key_env) && env_value_present(&secret_key_env) {
        ...
        match (
            optional_env_value("DEVBOX_R2_ENDPOINT"),
            optional_env_value("DEVBOX_R2_BUCKET"),
        ) {
            (Some(endpoint), Some(bucket)) => Ok(config.with_default_object_store(
                ManagedObjectProviderKind::R2,
                endpoint,
                bucket,
                optional_env_value("DEVBOX_R2_REGION").unwrap_or_else(|| "auto".to_string()),
            )?),
            (None, None) => Ok(config),
            _ => Err("DEVBOX_R2_ENDPOINT and DEVBOX_R2_BUCKET must be set together".into()),
        }
    } else {
